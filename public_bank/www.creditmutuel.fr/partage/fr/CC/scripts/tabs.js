        ...
        $("[data-tabs=yes]").each(function(t){
			...
			tabs.children().each(function(i){
				...
				if (i==0) {
					...
					tabsList += '<li role="presentation"><a id="tab'+(t+1)+'-'+(i+1)+'" role="tab" aria-controls="panel'+(t+1)+'-'+(i+1)+'" tabindex="0" aria-selected="false" data-nTab="'+(i+1)+'">'+$(this).find(titlesTag).html()+'</li>';
				}
			});
			tabsList += '</ul>';
			...
			tabList.find('a').click(function() {
				tabList.children(':eq('+(nTab-1)+')').removeClass('sel').find("a").attr("tabindex","0").attr("aria-selected","true");
          ...
          if(e.keyCode === 32) {
            console.log(tabs.children(':eq(0)').find('a:focus'))
            e.preventDefault();
            tabs.children(':eq(0)').find('a:focus').click();
          }
				} else {
					if ((e.keyCode == 37) || (e.keyCode == 38)) { // Gauche ou haut
						e.preventDefault();
