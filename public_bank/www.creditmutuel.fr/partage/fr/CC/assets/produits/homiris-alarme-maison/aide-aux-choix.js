...
(function() {
    ...
    function SaveSimul(typeHabitation,natureOccupant,intervention,protectionAgression,protectionIncendie,protectionMonoxydeCarbone,optionGsm) {
		var url = "/fr/webservices/EPS/Enregistrer.cgi";
		//console.log("Enregistrer url:  " + url);
    	var data = JSON.stringify({"typeHabitation": typeHabitation, "natureOccupant": natureOccupant, "intervention": intervention, "protectionAgression": protectionAgression, "protectionIncendie": protectionIncendie, "protectionMonoxydeCarbone": protectionMonoxydeCarbone, "optionGsm": optionGsm});
		//console.log("Enregistrer data: " + data);
		$.ajax({
			type: "POST",
			async: false,
			url: url,
			...
			complete: function(data,res){
				...
				if (coderetour == -1) {
					...
					//console.log("Ajax Erreur:" + messageErreur);
				}
				$('#btnSaveSimul').hide();
				$('#result .actions').prepend('<p class="msg info">Votre simulation a bien &eacute;t&eacute; sauvegard&eacute;e</p>');

				// Analytics
				//console.log("Analytics: " + 'sauvegarder');
    ...
    // Fonction permettant d'appeler le webservice EPS pour r�cuperer une simulation
    function GetSimul() {
		...
		.done(function( datajsonUS ) {
			...
			if (UserStatus == "authentifie") {
				...
				//console.log("RecupSimul url: " + url);
				$.ajax({
					type: "GET",
					async: false,
					url: url,
					...
					complete: function(data,res){
						...
						if (rep.reponse.simulationExiste) {
							...
							$("button#INPNext6").attr('disabled', false);
							if ( simul.optionGsm ) {
								$("input[name=amcRdStep7][value=1]").prop('checked', true);
							} else {
								$("input[name=amcRdStep7][value=2]").prop('checked', true);
							}
							...
							sliderInner.animate({
								...
								sliderContainer.find('progress div').width(((nSlide)*(100/7))+'%');

							});

							$("button#INPSubmit7").attr('disabled', false).click();

							// Analytics
							//console.log("Analytics: " + 'recuperer-sauvegarde');
    ...
    function initSlider(e) {
		...
		// R�ponse de l'aide au choix
		sliderContainer.find('[name=INPSubmit]').click(function() {
			var host = window.location.host;
			//if(host.includes("cic.")){
				//bug : includes n'est pas g�r� par IE ... 
				//pose soucis sur CMO et IE
				//bugfix : remplac� par indexOf
			if(!!~host.indexOf("cic.")) {
				//teste si cic dans le nom de domaine
				url = "telesurveillance-maison-homiris.cgi?";
			}else{
				url = "alarme-maison-homiris.cgi?";
			}
			...
			rdStep7 = $("input[name=amcRdStep7]:checked").attr('value');
        	url = url + 'amcJson=1&amcQ=7&amcRdStep1=' + rdStep1 + '&amcRdStep2=' + rdStep2 + '&amcRdStep3=' + rdStep3 + '&amcRdStep4=' + rdStep4 + '&amcRdStep5=' + rdStep5 + '&amcRdStep6=' + rdStep6 + '&amcRdStep7=' + rdStep7;
			...
			jQuery.getJSON(url, function(datajson) {
				...
				.done(function( datajsonUS ) {
					...
					if (UserStatus == "authentifie") {
						if (simulationExiste) {
							// L'utilisateur est authentifi� et vient de reprendre une simulation
							...
							// L'utilisateur est authentifi� et vient de finir une simulation
					...
					// La formulaire pr�conis�e --------------------------------
					//console.log("Formule pr�conis�e: " + datajson.result[2] );
					...
					$('#btnSaveSimul').click(function() {
						...
						optionGsm = ($("input[name=amcRdStep7]:checked").attr('value')==1?true:false);
						SaveSimul(typeHabitation,natureOccupant,intervention,protectionAgression,protectionIncendie,protectionMonoxydeCarbone,optionGsm);
		...
		// R�cup�ration de variables dans la QS
...
console.log("Variable 1: " + getVar("amcRdStep1") );
			...
			// R�cup�ration d'une sauvegarde pr�c�dente
			GetSimul();
		} */

		GetSimul();

		// Datalayer question-0 remontant à l'initialisation du module
		if (typeof dataLayer !== 'undefined') {
			dataLayer.push({'customCategory':'Technique', 'customAction':'PageVirtuelle', 'customLabel':'question-0','event':'customEvent'});
		}
    }
